...
const commands = async (
    ...
    commandlogger.info(
        `[#${cmdArgs.channel}] <${cmdArgs.username}>: request command - ${command}; args - ${message}`,
    );
    ...
    if (command in localcommandslist) {
        ...
        try {
            ...
            commandlogger.info(
                `[#${cmdArgs.channel}] <${cmdArgs.username}>: response command - ${command}; args - ${response.data.result}`,
            );
        } catch (error) {
            ...
            commandlogger.info(
                `[#${cmdArgs.channel}] <${cmdArgs.username}>: command error - ${command}; args - ${error}`,
            ); 
        }
    } else {
